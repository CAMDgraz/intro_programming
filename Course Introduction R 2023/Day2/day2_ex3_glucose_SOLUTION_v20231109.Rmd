...
for `r nrow(dt_glucose)`` patients.

```{r}
# TO DO: Show first few lines
...
```{r}
 ...
 dt_glucose %>%
  tbl_summary(include = c(Race, Sex, glucose_diff, age))
...
dt_glucose %>%
  tbl_summary(include = c(Race, glucose_diff, age),
              by = Sex)
...
dt_glucose %>% 
  ...
  group_by(Sex) %>% 
  mutate(mean_glucose_diff = mean(glucose_diff)) %>% 
  ungroup %>% 
  filter(row_number() <= 6) %>% 
  flextable::flextable()
